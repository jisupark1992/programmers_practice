...
public class 오픈_채팅방 {
    ...
    *       * 채팅방에서 닉네임을 변경한다.
    *   닉네임을 변경할 때는 기존에 채팅방에 출력되어 있던 메시지의 닉네임도 전부 변경된다.예를 들어,
    ...
    *
    *   - 시간 복잡도 분석
    *   N은 record의 길이입니다. 첫 번째 반복문에서 record의 모든 항목을 순회하므로 시간 복잡도는 O(N)이고, 두 번째 반복문의 시간 복잡도는
    *   O(N)입니다. 따라서 최종 시간 복잡도는 O(N)입니다.
    */
